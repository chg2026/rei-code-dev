...
export async function GET() {
  try {
    ...
    // Last-resort guard: never let an unexpected throw turn into an opaque
    // 500 that the login client can't interpret. Return a clean 500 body.
    console.error("[auth/user] unexpected failure", err);
    return NextResponse.json(
      { investor: null, error: "server_error", message: "Sign-in failed. Please try again." },
      { status: 500 }
    );
  }
}

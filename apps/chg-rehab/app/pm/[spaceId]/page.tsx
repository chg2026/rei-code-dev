...
export default async function PmSpacePage({
  params,
}: {
  ...
  if (space.lists.length > 0) {
    redirect(`/pm/${spaceId}/${space.lists[0].id}`);
  }

  return (
    <PmLayout spaces={spaces} selectedSpaceId={spaceId} statuses={space.statuses} lists={space.lists}>

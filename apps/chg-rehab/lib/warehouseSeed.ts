...
/**
 * Standard warehouse catalog (8 departments, 24 subcategories, ~195 prototype
 * items, 3 templates) and the idempotent seeding routine shared by the CLI
 * seed script and the warehouse page auto-heal.
 */
...
export async function seedWarehouseForCompany(
  db: PrismaClient,
  companyId: string,
  opts: { reset?: boolean } = {},
) {
  const RESET_ITEMS = opts.reset ?? false;
  ...
  // Seed inside one transaction guarded by a per-company Postgres advisory
  // lock. The warehouse page can trigger this from a GET render, so two
  // simultaneous first-loads of a brand-new (empty) company could otherwise
  // both pass the "0 departments" check and double-insert the catalog. The
  // xact lock serializes them: the second caller blocks until the first
  // commits, then finds the subcategories non-empty and skips item creation.
  // Running everything in one transaction also makes the seed atomic — a
  // mid-seed failure rolls back instead of leaving a partial catalog that the
  // (non-reset) idempotency check would never repair.
  const result = await db.$transaction(
    async (tx) => {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`wh-seed:${companyId}`})::bigint)`;
      ...
      for (const d of WH_DEPARTMENTS) {
        const dep = await tx.warehouseDepartment.upsert({
        ...
        for (const s of d.subs) {
          const sub = await tx.warehouseSubcategory.upsert({
          ...
          const existingItems = await tx.warehouseItem.count({
          ...
          if (RESET_ITEMS) {
            await tx.warehouseItem.deleteMany({ where: { subcategoryId: sub.id } });
          }
      ...
      for (const t of TEMPLATES) {
        const exists = await tx.warehouseTemplate.findFirst({
          where: { companyId, name: t.name },
        });
        if (!exists) {
          await tx.warehouseTemplate.create({
            data: {
              companyId,
              name: t.name,
              isLocked: t.isLocked,
      ...
      return {
        departments: WH_DEPARTMENTS.length,
  ...
  console.log(
    `[wh-seed] done: ${result.departments} departments, ${result.subcategories} subcategories, ${result.items} items, ${result.templatesAdded} new templates`
  );
  return result;
}

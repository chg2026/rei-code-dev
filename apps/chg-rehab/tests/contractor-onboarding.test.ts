...
import { invitationIsActivatable, invitationIsResendable, invitationIsRevokable } from "@/components/rehab/ContractorOnboarding";
...
describe("contractor onboarding invitation actions", () => {
  ...
  it("allows revoke only for a non-expired pending invitation", () => {
    const future = new Date(Date.now() + 60_000).toISOString();
    expect(invitationIsRevokable({ status: "Pending", expiresAt: future })).toBe(true);
    expect(invitationIsRevokable({ status: "Accepted", expiresAt: future })).toBe(false);
    expect(invitationIsRevokable({ status: "Pending", expiresAt: new Date(Date.now() - 1).toISOString() })).toBe(false);
  });
